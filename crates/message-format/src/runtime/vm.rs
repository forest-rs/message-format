...
use crate::runtime::{
    ...
    value::{Args, ResolvedString, StrId, StringDirection, Value},
};
...
fn store_value(values: &mut Vec<StoredValue>, value: Value) -> usize {
    store_value_with_id(values, value, None)
}
...
fn stored(values: &[StoredValue], id: usize) -> Result<&StoredValue, FormatError> {
    values
        .get(id)
...
/// The VM also consults [`Host::format_default_to`] for plain interpolation output.
pub trait Host {
    ...
    /// host may still return a value when formatting can continue.
    fn call(
        &mut self,
        catalog: &Catalog,
    ...
    /// Recoverable diagnostics are reported through `on_error`.
    /// Returning [`Value::Null`] disables matching and makes the VM select the
    /// default arm; this is useful when a recoverable option error leaves a
    /// value suitable for formatting but not for selection.
    fn call_select(
        &mut self,
        catalog: &Catalog,
        index: &Self::CatalogIndex,
    ...
    /// Write a host-specific default interpolation directly to `sink`.
    ///
    /// The default preserves [`Host::format_default`] behavior. Return `true`
    /// after handling the value and writing any output; return `false`
    /// without writing when the VM should use its built-in rendering. Hosts
    /// can override this entry point to avoid allocating an owned string.
...
impl<H: Host + ?Sized> Host for Box<H> {
    ...
    fn format_default_to(
        ...
        H::format_default_to(self, catalog, index, value, sink)
    }
}
...
struct SinkForward<'a, S: FormatSink + ?Sized> {
    sink: &'a mut S,
    id: Option<&'a str>,
}

impl<'a, S: FormatSink + ?Sized> SinkForward<'a, S> {
    fn new(sink: &'a mut S) -> Self {
        Self { sink, id: None }
    }

    fn identified(sink: &'a mut S, id: &'a str) -> Self {
        Self { sink, id: Some(id) }
    }
}
...
impl<S: FormatSink + ?Sized> FormatSink for SinkForward<'_, S> {
    ...
    fn formatted_value(&mut self, value: &FormattedValue<'_>) {
        if let Some(id) = self.id {
            ...
            self.sink.formatted_value(value);
        }
    }
        ...
        match value {
            ...
            Value::Fallback(id) => {
                catalog
                    .pool_string_opt(*id)
                    .map_or(Self::InvalidBorrowed, |text| Self::Borrowed {
                        view: ValueView::Fallback(text),
                        str_id: None,
                    })
            }
            Value::FunctionFallback(id) => {
                catalog
                    .pool_string_opt(*id)
                    .map_or(Self::InvalidBorrowed, |text| Self::Borrowed {
                        view: ValueView::Fallback(text),
...
impl ExprState {
    ...
    fn take_function_fallback(&mut self, catalog: &Catalog) -> Result<Value, FormatError> {
        let Some(fallback_id) = self.fallback_id.take() else {
            return Ok(Value::Null);
        };
        catalog
            .string(fallback_id)
            .map_err(|_| FormatError::Trap(Trap::InvalidFallbackStringId))?;
        Ok(Value::FunctionFallback(fallback_id))
    }
        ...
        let value_id = if function_failed {
            if let Some(fallback_id) = self.fallback_id.take() {
                catalog
                    .string(fallback_id)
                    .map_err(|_| FormatError::Trap(Trap::InvalidFallbackStringId))?;
                store_value(values, Value::FunctionFallback(fallback_id))
            } else {
                value_id
            }
        } else if failed {
    ...
    loop {
        ...
        match opcode {
            ...
            Opcode::ResolveString => {
                ...
                let id = stored(values, value)?.id.clone();
                stack.push(store_value_with_id(values, resolved, id));
            }
            Opcode::OutLit
            | Opcode::OutSlice
            | Opcode::OutExpr
...
fn local_slot(code: &[u8], base: usize) -> Result<usize, FormatError> {
    usize::try_from(read_u32(code, base + 1)?)
        .map_err(|_| FormatError::Trap(Trap::InvalidLocalSlot))
}
...
fn handle_select_instruction<'a>(
    ...
    match opcode {
        ...
        Opcode::SelectStringArg => {
            let key_id = read_u32(code, base + 1)?;
            *selector = Some(load_string_selector_value(
                args,
                catalog,
                key_id,
                diagnostics,
...
fn load_string_selector_value<'a>(
    args: &'a dyn Args,
    catalog: &'a Catalog,
    key_id: StrId,
    diagnostics: &mut Option<&mut dyn DiagnosticsSink>,
    ...
    if let Some(value) = args.get_ref(key_id) {
        if matches!(value, Value::Null) {
            // The optionless string function resolves a present null value to
            // empty text. Keep a missing argument distinct: it remains an
            // invalid selector and records `MissingArg` below.
            SelectorValue::ExactText("")
        } else {
            SelectorValue::from_borrowed(value, catalog)
        }
    } else {
...
fn resolve_optionless_string(
    ...
    let resolved = if let Value::String(value) = value {
        value.clone()
        ...
        match ValueView::from_value(value, catalog).map(ValueView::format_display) {
            Some(Cow::Borrowed(text)) => ResolvedString::plain_borrowed(text),
            Some(Cow::Owned(text)) => ResolvedString::plain_owned(text),
...
fn handle_call_opcode<H: Host>(
    ...
    let inherited_id = if structured_output && arg_count > 0 {
        ...
        stored(values, *first_arg)?.id.clone()
    } else {
        None
    };
    let (result, id) = if arg_count == 1 {
...
fn handle_project_select<H: Host>(
    ...
    if matches!(value, Value::FunctionFallback(_)) {
        stack.push(store_value(values, Value::Null));
        return Ok(());
    }
    ...
    let result = match host.project_select(catalog, index, fn_id, value, &mut on_error) {
        ...
        Err(_error) => {
            // `ProjectSelect` follows an explicit selector check. The
            // projection contributes the selector failure, while the host's
            // implementation detail must not surface as a second diagnostic.
            record_bad_selector(diagnostics, None);
            Value::Null
        }
    };
    ...
    if has_pending_operand_error || has_propagated_fallback {
        ...
        let id = if opcode == Opcode::CallFunc
            ...
            resolved_call_id(fn_id, call_options, catalog)
                ...
                .or_else(|| inherited_id.map(Into::into))
        } else {
            None
        };
        Ok((result, id))
    ...
    match call_result {
        Ok(result) => {
            ...
            if opcode == Opcode::CallSelect && matches!(result, Value::Null) {
                ...
                // takes the default arm without inventing a second option
                // error.
                record_bad_selector(diagnostics, None);
            }
            Ok(result)
...
impl<'a> ValueView<'a> {
    ...
    fn matches_case(self, case: &str) -> bool {
        self.case_match(case) != CaseMatch::No
    }

    fn case_match(self, case: &str) -> CaseMatch {
        match self {
            Self::Null => CaseMatch::No,
            Self::Bool(v) => {
                if (if v { "true" } else { "false" }) == case {
                    CaseMatch::Exact
                } else {
                    CaseMatch::No
                }
            ...
            Self::Float(v) => {
                if string_value_matches_case(&v.to_string(), case) {
                    CaseMatch::Exact
                } else {
                    CaseMatch::No
                }
    ...
    fn string_case_match(self, case: &str) -> CaseMatch {
        let matches = match self {
            Self::Null | Self::Fallback(_) => false,
            Self::Bool(value) => (if value { "true" } else { "false" }) == case,
            Self::Int(value) => int_matches_case(value, case),
            Self::Float(value) => value.to_string() == case,
...
fn emit_output_value<H: Host, S: FormatSink + ?Sized>(
    ...
    let mut forwarded = match id {
        Some(id) => SinkForward::identified(sink, id),
        None => SinkForward::new(sink),
    };
    if !host.format_default_to(catalog, index, value, &mut forwarded) {
        emit_value_ref(&mut forwarded, catalog, value);
    }
}
...
fn emit_arg_direct_or_fallback<H: Host, S: FormatSink + ?Sized>(
    ...
    if let Some(value) = args.get_ref(key_id) {
        emit_output_value(sink, host, index, catalog, value, None);
        return;
    }
...
pub(crate) fn format_i64(value: i64) -> IntText {
...
mod tests {
    ...
    fn host_remains_object_safe() {
        ...
        let _host: &mut dyn Host<CatalogIndex = ()> = &mut host;
    }
    ...
    #[test]
    fn default_project_select_delegates_with_the_stored_value() {
        #[derive(Default)]
        struct DefaultProjectionHost;

        impl Host for DefaultProjectionHost {
            type CatalogIndex = ();
            ...
            fn call(
                ...
                panic!("projection should delegate to call_select")
            }
        ...
        let code = TestOps::new()
            ...
            .build();
        let catalog = catalog_for_test(&["main", "value", "one"], "hitother", &code);
        let mut formatter = Formatter::new(&catalog, DefaultProjectionHost).expect("host");
        let mut sink = String::new();
        let errors = formatter
            .format_to_for_test_by_id(
                "main",
    ...
    #[test]
    fn local_store_load_round_trip_and_replace() {
        let code = TestOps::new()
            .push_const(1)
            .store_local(0)
            .push_const(2)
            ...
            .halt()
            .build();
        let catalog = catalog_for_test(&["main", "first", "second"], "", &code);
        let mut formatter = formatter_noop(&catalog);
        let out = formatter
            .format_by_id_for_test("main", &[])
            .expect("formatted");
    ...
    fn local_slots_are_fresh_for_each_invocation() {
        ...
        assert_eq!(
            Catalog::from_bytes(&catalog_bytes).expect_err("uninitialized local must reject"),
    ...
    #[test]
    fn invalid_local_slots_are_rejected_without_resizing() {
        for slot in [0, u32::MAX] {
            let code = TestOps::new().load_local(slot).out_val().halt().build();
            ...
            assert_eq!(
                Catalog::from_bytes(&bytes).expect_err("invalid local must reject"),
                CatalogError::InvalidLocalSlot { pc: 0, slot }
            );
        }
        ...
        assert_eq!(
            Catalog::from_bytes(&bytes).expect_err("huge store must reject"),
            CatalogError::InvalidLocalSlot {
                pc: 5,
                slot: u32::MAX,
            }
    ...
    #[test]
    fn local_binding_finalizes_missing_arg_fallback_and_diagnostic() {
        let code = TestOps::new()
            .load_arg(1)
            .expr_fallback(2)
            .store_local(0)
            ...
            .halt()
            .build();
        let catalog = catalog_for_test(&["main", "missing", "{$missing}"], "", &code);
        let mut formatter = formatter_noop(&catalog);
        let mut sink = String::new();
        let errors = formatter
            .format_to_for_test_by_id("main", &[], &mut sink)
    ...
    #[test]
    fn fallback_local_is_not_a_selector_and_reports_bad_selector() {
        let code = TestOps::new()
            .load_arg(1)
            .expr_fallback(2)
            .store_local(0)
            ...
            .halt()
            .build();
        let catalog = catalog_for_test(&["main", "missing", "{$missing}", "hit"], "Xother", &code);
        let mut formatter = formatter_noop(&catalog);
        let mut sink = String::new();
        let errors = formatter
            .format_to_for_test_by_id("main", &[], &mut sink)
    ...
    fn invalid_stored_number_selection_does_not_match_and_reports_once() {
        ...
        let errors = formatter
            ...
            .expect("formatted");
        assert_eq!(sink, "other");
        assert_eq!(errors, vec![FormatError::BadSelector { source: None }]);
    }
