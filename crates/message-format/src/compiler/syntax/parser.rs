...
pub(crate) fn validate_known_declaration_heads(
    source: &str,
    ctx: crate::compiler::syntax::span::SourceContext,
) -> Result<(), CompileError> {
    ...
    Err(CompileError::invalid_expr_detail(
        ...
        crate::compiler::syntax::span::quoted_snippet(&body_src[..end]),
    ))
}
...
mod tests {
    ...
    fn parses_markup_close_tag() {
        ...
        if let Some(ExpressionPayloadNode::Markup(m)) = &exprs[0].payload {
            assert_eq!(m.kind, crate::compiler::syntax::ast::MarkupKind::Close);
    ...
    fn parses_markup_self_close_tag() {
        ...
        assert_eq!(exprs.len(), 1);
        if let Some(ExpressionPayloadNode::Markup(m)) = &exprs[0].payload {
            assert_eq!(m.kind, crate::compiler::syntax::ast::MarkupKind::SelfClose);
