...
use crate::compiler::syntax::parser::parse_document;
use crate::compiler::syntax::span::{SourceContext, byte_to_line_col, quoted_snippet};
...
pub(crate) struct SemanticExpression<'a> {
    pub(crate) node: crate::compiler::syntax::ast::ExpressionNode<'a>,
}
...
pub(crate) struct MatchVariantPrelude<'a> {
    ...
    pub(crate) pattern: crate::compiler::syntax::ast::PatternNode<'a>,
}
...
pub(crate) fn parse_match_declaration_prelude_from_document<'a>(
    ...
    for selector in selectors {
        let Some(crate::compiler::syntax::ast::ExpressionPayloadNode::Var(var)) = &selector.payload
        else {
...
fn parse_input_declared_source_name(
    ...
    let var = match &expr.payload {
        Some(crate::compiler::syntax::ast::ExpressionPayloadNode::Var(var)) => var,
        Some(crate::compiler::syntax::ast::ExpressionPayloadNode::Call(call)) => {
            match &call.operand {
                crate::compiler::syntax::ast::CallOperandNode::Var(var) => var,
    ...
    let start = trimmed
        .char_indices()
        .find(|(_, ch)| {
            !is_bidi_control(*ch) && !crate::compiler::syntax::charset::is_mf2_whitespace(*ch)
        })
        .map_or(0, |(idx, _)| idx);
    let end = trimmed
        .char_indices()
...
fn collect_var_refs_from_node(
    ...
    match payload {
        ...
        crate::compiler::syntax::ast::ExpressionPayloadNode::Call(call) => {
            if let crate::compiler::syntax::ast::CallOperandNode::Var(var) = &call.operand {
                collect_var_ref(var.name, out);
            }
        ...
        crate::compiler::syntax::ast::ExpressionPayloadNode::Markup(_) => {}
    }
}
...
fn compile_error_from_match_parse_error(
    ...
    use crate::compiler::syntax::ast::MatchParseError as E;
    let err = decl
        .match_error
        .as_ref()
...
mod tests {
    ...
    use crate::compiler::syntax::ast::ExpressionPayloadNode;
    use crate::compiler::syntax::span::SourceContext;
    ...
    fn rejects_local_missing_name() {
        ...
        assert!(matches!(
            err,
            crate::compiler::compile::CompileError::InvalidExpr { .. }
        ));
    }
    ...
    fn match_missing_selector_reports_selector_diagnostic() {
        ...
        let err = parse_match_declaration_prelude(src, line_ctx(1)).expect_err("must fail");
        match err {
            crate::compiler::compile::CompileError::InvalidExpr { expected, .. } => {
    ...
    fn match_malformed_key_reports_key_diagnostic_consuming() {
        ...
        let err = parse_match_declaration_prelude(src, line_ctx(1)).expect_err("must fail");
        match err {
            crate::compiler::compile::CompileError::InvalidVariantKey {
                expected, found, ..
            } => {
    ...
    fn rejects_duplicate_normalized_variant_keys() {
        ...
        assert!(matches!(
            err,
            crate::compiler::compile::CompileError::DuplicateVariant { .. }
        ));
    }
    ...
    fn parse_prefixed_variable_token_rejects_invalid_identifier() {
        ...
        match err {
            crate::compiler::compile::CompileError::InvalidVar {
                line,
                column,
                expected,
                found,
    ...
    fn parse_function_spec_node_reports_missing_equals_for_known_function() {
        ...
        let function = crate::compiler::syntax::ast::FunctionSpecNode {
            name: "number",
            name_span: 0..6,
