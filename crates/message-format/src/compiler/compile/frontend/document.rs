...
pub(super) fn parse_single_message(
    ...
    if let Some(match_prelude) =
        crate::compiler::syntax::semantic::parse_match_declaration_prelude_from_document(
            source,
            &analyzed.doc,
            ctx,
        )?
...
pub(super) fn validate_and_parse_declarations(
    source: &str,
    ctx: SourceContext,
) -> Result<crate::compiler::syntax::semantic::CanonicalDeclarationPrelude<'_>, CompileError> {
    crate::compiler::syntax::parser::validate_known_declaration_heads(source, ctx)?;
    let declarations = crate::compiler::syntax::semantic::parse_declaration_prelude(source, ctx)?;
    crate::compiler::syntax::semantic::validate_declaration_scope(source, &declarations, ctx)?;
    crate::compiler::syntax::semantic::validate_no_implicit_redeclaration(source, ctx)?;
    crate::compiler::syntax::semantic::canonicalize_declaration_prelude(source, declarations, ctx)
}
...
fn preprocess_single_message_parts(
    ...
    let pattern_node = crate::compiler::syntax::parser::parse_pattern(pattern);
...
fn split_declarations_and_body_from_doc<'a>(
    source: &'a str,
    doc: &crate::compiler::syntax::ast::SyntaxDocument<'_>,
) -> (&'a str, &'a str) {
